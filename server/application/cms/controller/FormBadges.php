 ...
 */
class FormBadges extends CmsBase
{
    protected $model;
    ...
    public function index(Request $request){
        ...
        if ($request->isPost()) {
            $list = $this->model->getCmsDatasForPage($curr_page, $this->page_limit, $search,$eventId,'Badge');
            return showMsg(1, 'success', $list);
        } else {
            $articles = $this->model->getCmsDatasForPage($curr_page, $this->page_limit, $search,$eventId,'Badge');
            $record_num = $this->model->getCmsDatasCount($search,$eventId,'Badge');
            $data = [
                'articles' => $articles,
                'search' => $search,
...
//            }
    ...
    public function download(Request $request){
        ...
        $record_num1 = $this->badgeModel->getVendorReportCount($eventId,$formId);
        $articles1 = $this->badgeModel->getVendorReportForPage($eventId,$formId,1, $record_num1);
...
//            $objPHPExcel->getActiveSheet()->setCellValue('Y'.($k+1+$topNumber),$v['badge_submit_timestamp']);
    ...
    public function report(Request $request)
    {
        ...
        if ($request->isPost()) {
            $articles = $this->badgeModel->getVendorReportForPage($eventId,$formId,$curr_page, $limit);
            $record_num = $this->badgeModel->getVendorReportCount($eventId,$formId);
            $data = [
                'articles' => $articles,
                'record_num' => $record_num
            ];
    ...
    public function getOrderItems(Request $request){
        if ($request->isPost()) {
            $eventId = $request->param('event_id');
            $id = $request->param('id');
            $options = $this->catalogModel->getUsedCmsList($eventId,'Badge');
            if(isset($id)){
