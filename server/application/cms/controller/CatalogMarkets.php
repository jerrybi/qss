 ...
 */
class CatalogMarkets extends CmsBase
{
    protected $model;
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isPost()) {
            $list = $this->model->getCmsDatasForPage($curr_page, $this->page_limit, $search,$event_id,'Marketing');
            ...
            $record_num = $this->model->getCmsDatasCount($search,'Marketing');
            $catalogAttrs = $this->catalogAttrModel->getCmsList($eventId,'Marketing');
    ...
    public function add(Request $request)
    {
        if ($request->isPost()) {
            ...
            $event = new Xevents();
            $events = $event->getSimpleEventsList();
            return view('add',['events'=>$events,'type'=>'Marketing']);
        }
    }
    ...
    public function fieldList(Request $request)
    {
        ...
        $event_id = $request->param('event_id');
        if ($request->isPost()) {
            $list = $this->model->getCmsDatasForPage($curr_page, 1000, $search,'Marketing');
            return showMsg(1, 'success', $list);
        } else {
            $event = new Xevents();
            $events = $event->getSimpleEventsList();
    ...
    public function editField(Request $request, $id='',$event_id='')
    {
        if ($request->isPost()) {
            ...
            $article = $this->catalogAttrModel->getDataByKey($id,$event_id,'Marketing');
            if(empty($article)){
                $name = '';
    ...
    public function getMainCategories(Request $request){
        if ($request->isPost()) {
            $eventId = $request->param('event_id');
            $article = $this->catalogAttrModel->getDataByKey('category',$eventId,'Marketing');
            $options = [];
            if(!empty($article)){
                $str = $article['options'];
                $options = explode("\r\n",$str);
    ...
    public function getUsedMainCategories(Request $request){
        if ($request->isPost()) {
            $eventId = $request->param('event_id');
            $options = $this->model->getUsedCmsList($eventId,'Marketing');
            return showMsg(1,'success',$options);
        }else {
            return showMsg(0, 'sorry,your request is invalid！');
        }
    ...
    public function getSubCategories(Request $request){
        if ($request->isPost()) {
            ...
            $options = $this->catalogAttrModel->getSubCategories($eventId,$category,'Marketing');
            return showMsg(1,'success',$options);
        }else {
            return showMsg(0, 'sorry,your request is invalid！');
        }
    ...
    public function download(Request $request){
        $eventId = $request->param('event_id');
        $record_num = $this->model->getCmsDatasCount('','Marketing');
        $articles = $this->model->getCmsDatasForPage(1, $record_num, '',$eventId,'Marketing');
