 ...
 */
class CatalogManpowers extends CmsBase
{
    protected $model;
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isPost()) {
            $list = $this->model->getCmsDatasForPage($curr_page, $this->page_limit, $search,$event_id,'Manpower');
            ...
            $record_num = $this->model->getCmsDatasCount($search,'Manpower');
            $catalogAttrs = $this->catalogAttrModel->getCmsList($eventId,'Manpower');
    ...
    public function add(Request $request)
    {
        if ($request->isPost()) {
            ...
            $event = new Xevents();
            $events = $event->getSimpleEventsList();
            return view('add',['events'=>$events,'type'=>'Manpower']);
        }
    }
    ...
    public function fieldList(Request $request)
    {
        ...
        $event_id = $request->param('event_id');
        if ($request->isPost()) {
            $list = $this->model->getCmsDatasForPage($curr_page, 1000, $search,'Manpower');
            return showMsg(1, 'success', $list);
        } else {
            $event = new Xevents();
            $events = $event->getSimpleEventsList();
    ...
    public function editField(Request $request, $id='',$event_id='')
    {
        if ($request->isPost()) {
            ...
            $article = $this->catalogAttrModel->getDataByKey($id,$event_id,'Manpower');
            if(empty($article)){
                $name = '';
    ...
    public function getMainCategories(Request $request){
        if ($request->isPost()) {
            $eventId = $request->param('event_id');
            $article = $this->catalogAttrModel->getDataByKey('category',$eventId,'Manpower');
            $options = [];
            if(!empty($article)){
                $str = $article['options'];
                $options = explode("\r\n",$str);
    ...
    public function getUsedMainCategories(Request $request){
        if ($request->isPost()) {
            $eventId = $request->param('event_id');
            $options = $this->model->getUsedCmsList($eventId,'Manpower');
            return showMsg(1,'success',$options);
        }else {
            return showMsg(0, 'sorry,your request is invalid！');
        }
    ...
    public function getSubCategories(Request $request){
        if ($request->isPost()) {
            ...
            $options = $this->catalogAttrModel->getSubCategories($eventId,$category,'Manpower');
            return showMsg(1,'success',$options);
        }else {
            return showMsg(0, 'sorry,your request is invalid！');
        }
    ...
    public function download(Request $request){
        $eventId = $request->param('event_id');
        $record_num = $this->model->getCmsDatasCount('','Manpower');
        $articles = $this->model->getCmsDatasForPage(1, $record_num, '',$eventId,'Manpower');
