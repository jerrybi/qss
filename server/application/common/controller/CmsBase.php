...
class CmsBase extends Base
{
    ...
    public function initAuth()
    {
        ...
        if (!$cmsAID) {
            ...
            if ($sysConf->checkCmsIpAuth()){
                ...
                $authFlag = $checkAuth;
                $message = $checkAuth ? "You have the permission" : "Sorry,You don't have permission!";
            }else{
    ...
    public function getCmsAdminPagePermissions(){
        $cmsAID = IAuth::getAdminIDCurrLogged();
        $action = trim(strtolower(request()->action()));
        $request_url = trim(strtolower($_SERVER["REQUEST_URI"]));
        $authUrl = explode($action, $request_url)[0] . $action;
        if(strpos($authUrl,'/')===0&&$authUrl !== '/'){
            $authUrl = substr($authUrl,1);
        }
        $navModel = new XnavMenus();
        $menuID = $navModel->getNavMenuIDByUrl($authUrl);
        $adminModel = new Xadmins();
        $permissions = $adminModel->getPagePermissions($cmsAID,$menuID);
        return $permissions;
    }
