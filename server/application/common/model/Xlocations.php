...
use app\common\validate\Xlocation;
use think\Db;
use \think\Model;
 ...
 * Time: 16:45
 */
class Xlocations extends BaseModel
{
    // 设置当前模型对应的完整数据表名称
    ...
    public function getLocationGroupByID($id)
    {
        $res = $this
            ->alias('a')
            ->field('a.location_group')
            ->where('a.status = 1')
            ->where("a.id = '" . $id."'")
            ->find();
        if(isset($res)){
            return $res['location_group'];
        }else{
            return '';
        }
    }
    ...
    public function getCmsDatasForPage($curr_page, $limit = 1, $search = null)
    {
        $res = $this
            ->alias('a')
            ->field('a.*,e.name as event_name,lg.name as location_group_name')
            ->join('xevents e','e.id = a.event_id')
            ->join('xlocationGroups lg','lg.id = a.location_group') 
    ...
    public function updateCmsData($input,$id = 0)
    {
        ...
        if ($opTag == 'del') {
            ...
            insertCmsOpLogs(1,'LOCATIONS',$id,'remove location');
        } else {
            $saveData = [
                'name' => isset($input['name'])?$input['name']:'',
                'description' => isset($input['description'])?$input['description']:'',
                'location_group' => isset($input['location_group'])?$input['location_group']:'',
            ...
            if ($validateRes['tag']) {
                ...
                if(!empty($result) && $result['id'] != $id){
                    return ['tag' => false, 'message' => 'Location Name Exist!'];
                }
                $saveTag = $this
                    ->where('id', $id)
                    ->update($saveData);
                if ($saveTag) {
                    insertCmsOpLogs($saveTag,'LOCATIONS',$id,'location groups');
                }
    ...
    public function addData($data)
    {
        ...
        $addData = [
            ...
            'location_group' => isset($data['location_group'])?$data['location_group']:'',
        ...
        if ($validateRes['tag']) {
            ...
            if ($tag) {
                insertCmsOpLogs($tag,'LOCATIONS',$this->getLastInsID(),'add location groups');
            }
            $validateRes['tag'] = $tag;
