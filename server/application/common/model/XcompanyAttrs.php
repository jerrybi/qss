...
use app\common\validate\XcompanyAttr;
use think\Db;

class XcompanyAttrs extends BaseModel
{
    private $validate;
    public function __construct($data = [])
    {
        parent::__construct($data);
        $this->validate = new xcompanyAttr();
    }
    ...
    public function updateCmsData($input,$id = 1)
    {
        ...
        if ($opTag == 'del') {
            ...
            insertCmsOpLogs(1,'company',$id,'remove company');
        } else {
            $saveData = [
    ...
    public function getData($id=1){
        return Db::name('xcompany_attrs')->where('id',$id)->find();
    }

    public function getDataByKey($key='',$eventId=''){
        return Db::name('xcompany_attrs')->where('key',$key)->where('event_id',$eventId)->find();
    }
}