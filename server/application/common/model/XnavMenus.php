...
//        $navMenus = explode('|', $str);
        $navMenus = $this->getTreeIds(json_decode($str,true));
    ...
    public function getNavMenusShow($cmsAID = 0)
    {
        if (!$cmsAID) {
            ...
            $str = $this->getAdminMenus($cmsAID);
//            $arr = explode('|', $str);
            $arr = $this->getTreeIds(json_decode($str,true));
    ...
    public function dealForAdminShowMenus($rootMenus, $arr)
    {
        foreach ($rootMenus as $key => $value) {
            ...
            if (!in_array($parent_id, $arr)) {
                ...
                if(empty($childRes) || count($childRes) == 0){
                    $rootMenus[$key]['child'] = FALSE;
                }else{
    ...
    public function getNavMenusForPage($curr_page, $limit, $search = null,$navType = "F")
    {
        ...
        foreach ($res as $key => $v) {
            ...
            $res[$key]['parent_name'] = $v['parent_id'] == 0 ? "Root Menu" : $parentData['name'];
       ...
       $opTag = isset($data['tag']) ? $data['tag'] : 'edit';
        ...
        if ($opTag == 'del') {
            $tag = $this
                ->where('id', $id)
                ->update(['status' => -1]);
            $validateRes['message'] = $tag ? 'Delete Success' : 'Already deleted';
        } else {
            $navType = isset($data['navType'])?$data['navType']:"F";
            $strNavType = "parent_id_".$navType;
            $parent_id = isset($data[$strNavType])?$data[$strNavType]:0;
            ...
            if ($validateRes['tag']) {
                $tag = $this
                    ->where('id', $id)
                    ->update($saveData);
                $validateRes['message'] = $tag ? 'Menu update success' : 'No change';
            }
    ...
    public function getNavMenuIDByUrl($url){
        return $this->where('action',$url)->where('status',1)->value('id');
    }
}