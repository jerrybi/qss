...
//                    $this->sendConfirmationMail($this->userInfo,$formId);
                    $task = [
                        ...
                        'data'=>json_encode(['type'=>'Amenity','user_id'=>$this->userInfo['unique_id'],'form_id'=>$formId]),
                        'status'=>0,
            ...
            foreach($orderItems as $category){
                $options = $this->catalogAttrModel->getSubCategories($this->userInfo['event_id'],$category,'Amenity');
                foreach($options as $value){
                    $res = $this->catalogModel->getCmsListByCategory($this->userInfo['event_id'],$category,$value,'Amenity');
                    if(count($res) > 0){
