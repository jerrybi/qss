...
class Manpower extends UserBase
{
    ...
    public function index(Request $request){
        if($request->isPost()){
            ...
            $res = $this->manpowerModel->addCmsData($this->userInfo['event_id'],$this->userInfo['company_id'],$formId,$request->param());
            if($res['tag']){
...
//                    $this->sendConfirmationMail($this->userInfo,$formId);
                    $task = [
                        ...
                        'data'=>json_encode(['type'=>'Manpower','user_id'=>$this->userInfo['unique_id'],'form_id'=>$formId]),
                        'status'=>0,
            ...
            foreach($orderItems as $category){
                $options = $this->catalogAttrModel->getSubCategories($this->userInfo['event_id'],$category,'Manpower');
                foreach($options as $value){
                    $res = $this->catalogModel->getCmsListByCategory($this->userInfo['event_id'],$category,$value,'Manpower');
                    if(count($res) > 0){
                        if(time() <= strtotime($form['early_submission_date'])){
                            foreach($res as $k=>$v){
                                $v['item_price'] = $v['advanced_rate']>0?$v['advanced_rate']:'Upon request';
                                $items[] = $v;
                            }
                        }else{
                            if(time() > strtotime($form['due_date'])){
                                foreach($res as $k=>$v){
                                    if($v['have_onsite_rate']){
                                        $v['item_price'] = $v['onsite_rate']>0?$v['onsite_rate']:'Upon request';
                                    }else{
                                        $v['item_price'] = $v['standard_rate']>0?$v['standard_rate']:'Upon request';
                                    }
                                    $items[] = $v;
                                }
                            }else{
            ...
            $formData = $this->manpowerModel->getLastData($this->userInfo['event_id'],$this->userInfo['company_id'],$formId);
            ...
            if(count($formData) > 0){
                ...
                if(empty($formData1['item_catalog_id'])){
                    $formData = [];
                }
            }else{
                $submitter = $this->userInfo['first_name'].' '.$this->userInfo['last_name'];
                $formData1 = ['language'=>'','form_id'=>$formId,
